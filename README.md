...
# Created by- Vaijanti Rajure © 2023
...
- Download this project, rename the main folder from `BasicBankingSystem-main` to `vaijantibanking`
- Now place the `vaijantibanking` folder in your XAMPP-->htdocs folder
...
- (*Note*- A dummy database will be also included, so you can use it as per your preference)

## How_To_Run
- After successfully importing database in the above step, open a new browser window and type the following:
``````````````
http://localhost/vaijantibanking/mainpage.php 
``````````````
